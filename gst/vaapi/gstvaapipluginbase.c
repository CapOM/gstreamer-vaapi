    ...
    GstQuery * query)
{
  ...
  if (has_texture_upload_meta) {
    ...
    if (params) {
      if (gst_structure_get (params, "gst.gl.GstGLContext", GST_GL_TYPE_CONTEXT,
        ...
        /* Fails texture upload meta method if downstrean can support dmabuf.
           I.e. prefer outputing dmabuf (with or without caps feature) over
           texture upload meta. By returning false it let to chance to output
           dmabuf, but it will fallback to the texture upload meta if it fails
           to output dmabuf with and without caps feature. In that case
           plugin->can_try_dmabuf will be false. */
  ...
  if (!pool || !gst_buffer_pool_has_option (pool,
    ...
    /* Only try to output dmabuf without caps feature for some video format.
       These video format are the same as those supported with the dmabuf caps
       feature. plugin->can_try_dmabuf will be false if dmabuf has already been
       tried at an earlier allocation query. */
    if (plugin->can_try_dmabuf
...
config_failed:
  {
    if (pool) {
      /* Do not error out if dmabuf without caps feature has failed. It can
         typically happen if the memory is not mappable. Returning false allows
         to try another caps. */
